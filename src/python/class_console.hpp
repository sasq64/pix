...
inline std::shared_ptr<FullConsole> make_console(int32_t cols, int32_t rows,
                                                 ...
                                                 Vec2f const& tile_size, int font_size)
{
    fs::path p{font_file};

    auto ts = std::pair<int, int>{tile_size.x, tile_size.y};
    auto font = font_file.empty() ? std::make_shared<TileSet>(FreetypeFont::unscii, ts)
...
inline void add_console_class(py::module_ const& mod)
{
    ...
    py::class_<FullConsole, std::shared_ptr<FullConsole>>(mod, "Console")
        .def(
            ...
            "be derived from the size of a character in the font with the provided `font_size`")
        .def(py::init<>(&make_console2), "cols"_a, "rows"_a, "tile_set"_a,
        ...
        .def_property("cursor_pos", &FullConsole::get_cursor, &FullConsole::set_cursor,
        ...
        .def("set_tiles", &FullConsole::set_tiles, "tiles"_a, "Set tiles from an array of ints.")
        ...
        .def_property_readonly("grid_size", &FullConsole::get_size, "Get number cols and rows")
        ...
        .def("read_line", &FullConsole::read_line,
             "Puts the console in line edit mode.\n\nA cursor will be shown and all text events "
             "will be captured by the console until `Enter` is pressed. At this point the entire "
             "line will be pushed as a `TextEvent`.")
        .def("cancel_line", &FullConsole::stop_line, "Stop line edit mode.")
        .def("set_line", &FullConsole::set_line, "text"_a, "Change the edited line.")
        .def("get_font_image", &FullConsole::get_font_texture)
        .def("get_image_for", &FullConsole::get_texture_for_char, "tile"_a,
             "Get the image of a specific tile. Use to render the tile manually, or to copy "
             "another image into the "
        ...
        .def(
            ...
            "tiles"_a)
        .def("write", static_cast<void (FullConsole::*)(std::string const&)>(&FullConsole::write),
             "text"_a,
