...
inline void add_vec2_class(py::module_& mod)
{
    ...
    vd.def(py::init<std::pair<double, double>>())
        ...
        .def_static("from_angle", &Vec2f::from_angle, py::arg("angle"), "Rotates the X-axis (1,0) around `angle` counter-clockwise and returns the result.")
        ...
        .def("angle", &Vec2f::angle, "Get the (counter-clockwise) angle between the vector and the X-axis (1,0).")
