...
template <typename Vec2> py::class_<Vec2> add_common(py::module_& mod, const char* name)
{
    auto vd =
        py::class_<Vec2>(mod, name)
            ...
            .def("__hash__", [](Vec2 const& v) { return (int)v.x + (int)(v.y * 65535); })
            ...
            .def_property_readonly("yx", [](Vec2 self) { return Vec2{self.y, self.x}; })
            .def_property_readonly("with_y0", [](Vec2 self) { return Vec2{self.x, 0}; })
            .def_property_readonly("with_x0", [](Vec2 self) { return Vec2{0, self.y}; })
            .def("__eq__", [](const Vec2& a, Vec2 const& b) { return a == b; })
            .def("__ne__", [](const Vec2& a, Vec2 const& b) { return a != b; })
            .def("__lt__", [](const Vec2& a, Vec2 const& b) { return a.x < b.x && a.y < b.y; })
            .def("__le__", [](const Vec2& a, Vec2 const& b) { return a.x <= b.x && a.y <= b.y; })
            .def("__gt__", [](const Vec2& a, Vec2 const& b) { return a.x > b.x && a.y > b.y; })
            .def("__ge__", [](const Vec2 a, Vec2 const& b) { return a.x >= b.x && a.y >= b.y; })
            .def("__getitem__",
            ...
            .def(
                "__iter__", [](Vec2 const& v) { return py::make_iterator(&v.x, &v.y + 1); },
                py::keep_alive<0, 1>());
...
inline void add_vec2_class(py::module_& mod)
{
    ...
    vd.def(py::init<std::pair<double, double>>())
        .def(
            "toi",
            [](Vec2f self) { return Vec2i{static_cast<int>(self.x), static_cast<int>(self.y)}; },
        ...
        .def("__repr__",
             [](Vec2f self) {
                 return "Float2(" + std::to_string(self.x) + ", " + std::to_string(self.y) + ")";
             })
        .def("__truediv__", &Vec2f::div)
        .def("__truediv__", [](Vec2f self, Vec2i other) { return self / Vec2f{other}; })
        .def("__truediv__", &Vec2f::divs)
        .def("__floordiv__", &Vec2f::fdiv)
        .def("__floordiv__", [](Vec2f self, Vec2i other) { return (self / Vec2f{other}).floor(); })
        .def("__floordiv__", &Vec2f::fdivs)
        .def("__mul__", &Vec2f::mul)
        .def("__mul__", [](Vec2f self, Vec2i other) { return self * Vec2f{other}; })
        .def("__mul__", &Vec2f::muls)
        .def("__add__", &Vec2f::add)
        .def("__add__", [](Vec2f self, Vec2i other) { return self + Vec2f{other}; })
        .def("__add__", &Vec2f::adds)
        .def("__sub__", &Vec2f::sub)
        .def("__sub__", [](Vec2f self, Vec2i other) { return self - Vec2f{other}; })
    ...
    vi.def(py::init<std::pair<int, int>>())
        .def(
            "tof",
            [](Vec2i self) {
                return Vec2f{static_cast<double>(self.x), static_cast<double>(self.y)};
            },
            "Convert an `Int2` to a `Float2`")
        .def(
            "random", [](Vec2i self) { return Vec2i{rand() % self.x, rand() % self.y}; },
            "Returns Int2(rnd(x), rnd(y)) where rnd(n) returns a random number between 0 and n.")
        .def("__repr__",
             [](Vec2i self) {
                 return "Int2(" + std::to_string(self.x) + ", " + std::to_string(self.y) + ")";
             })
        .def("__truediv__", &Vec2i::div)
        .def("__truediv__", [](Vec2i self, Vec2f other) { return Vec2f{self} / other; })
        .def("__truediv__", &Vec2i::divs)
        .def("__truediv__", [](Vec2i self, double other) { return Vec2f{self} / other; })
        .def("__floordiv__", &Vec2i::fdiv)
        .def("__floordiv__", [](Vec2i self, Vec2f other) { return (Vec2f{self} / other).floor(); })
        .def("__floordiv__", &Vec2i::fdivs)
        .def("__floordiv__",
             [](Vec2i self, double other) { return (Vec2f{self} / other).floor(); })
        .def("__mul__", &Vec2i::mul)
        .def("__mul__", [](Vec2i self, Vec2f other) { return Vec2f{self} * other; })
        .def("__mul__", &Vec2i::muls)
        .def("__mul__", [](Vec2i self, double other) { return Vec2f{self} * other; })
        .def("__add__", &Vec2i::add)
        .def("__add__", [](Vec2i self, Vec2f other) { return Vec2f{self} + other; })
        .def("__add__", &Vec2i::adds)
        .def("__add__", [](Vec2i self, double other) { return Vec2f{self} + other; })
        .def("__sub__", &Vec2i::sub)
        .def("__sub__", [](Vec2i self, Vec2f other) { return Vec2f{self} - other; })
        .def("__sub__", &Vec2i::subs)
        .def("__sub__", [](Vec2i self, double other) { return Vec2f{self} - other; })
