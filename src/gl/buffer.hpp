...
// of the buffer.
template <BufferTarget Target, GLenum Usage = GL_STATIC_DRAW> struct Buffer
{
    GLuint buffer = 0;
    size_t size = 0;
    ...
    explicit Buffer(int64_t size_in_bytes = 0)
    {
    ...
    void update(void* data, int64_t offset_in_bytes, int64_t size_in_bytes)
    {
        assert(size >= (offset_in_bytes + size_in_bytes));
        bind();
        glBufferSubData(to_glenum(Target), offset_in_bytes, size_in_bytes,
                        data);
    }
    ...
    // Bind this buffer and reset its content
    template <typename T, size_t N> void set(std::array<T, N> const& data)
    {
        glBufferData(to_glenum(Target), data.size() * sizeof(T), data.data(),
                     Usage);
        size = data.size() * sizeof(T);
    }

    // Bind this buffer and reset its content
    template <typename T> void set(std::vector<T> const& data)
    {
        glBufferData(to_glenum(Target), data.size() * sizeof(T), data.data(),
                     Usage);
        size = data.size() * sizeof(T);
